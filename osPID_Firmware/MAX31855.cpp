...
* ========  ===========
* x.xx      Modified for use specifically in the osPID.
...
* Description: Read the thermocouple temperature in Degree Celsius.
*							 Internally, the conversion takes place in the
...
ospDecimalValue<1> MAX31855::readThermocoupleCelsius()
{
  ...
  if (data & 0x00010000)
  {
    ...
    switch (data & 0x00000007)
    {
      // Open circuit
    case 0x01:
      return makeDecimal<1>(FAULT_OPEN);
      break;

      // Thermocouple short to GND
    case 0x02:
      return makeDecimal<1>(FAULT_SHORT_GND);
      break;

      // Thermocouple short to VCC
    case 0x04:
      return makeDecimal<1>(FAULT_SHORT_VCC);
      break;
    }
  }
  // No fault detected
  else
  {
    ...
    data = data >> 18;
    // Bit-14 is the sign
    int temperature = (data & 0x00001FFF);
    ...
    // temperature is in quarter-units, now we want to convert it to tenths
    // we multiply it by 10 to put it in 40ths, and then divide by 4 to put it in
    // 10ths
    ospDecimalValue<1> quarters = makeDecimal<1>(temperature * 10);
...
/*******************************************************************************
* Name: readJunction
* Description: Read the thermocouple temperature either in Degree Celsius.
*							 Internally, the conversion takes place in the
*							 background within 100 ms. Values are updated only when the CS
*							 line is high.
*
...
*
*******************************************************************************/
ospDecimalValue<2> MAX31855::readJunctionCelsius()
{
  unsigned long data;
  ...
  data = data >> 4;
  // Bit-12 is the sign
  int temperature = (data & 0x000007FF);
  ...
  // temperature is in 16ths -- convert it to rounded 100ths
  ospDecimalValue<1> sixteenths = makeDecimal<1>(temperature * 10);
  return (sixteenths / makeDecimal<2>(1600)).rescale<2>();
}
